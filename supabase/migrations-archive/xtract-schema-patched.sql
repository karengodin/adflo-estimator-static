-- =============================================================================
-- adfloXtract Patched Schema — Safe to run against existing Supabase project
-- =============================================================================
--
-- WHAT THIS FILE DOES:
...
--   The standard xtract-schema.sql (built from Lovable migrations) includes a
--   CREATE TABLE for "instances" — but that table already exists in this Supabase
--   project (used by adflo-implementation-tools for TapClicks credential storage).
--   Rather than drop/replace it, this file adds only the missing "user_id" column
--   and creates everything else from scratch.
--
-- WHAT ALREADY EXISTS (do not touch):
--   - instances          (exists, will receive one new column: user_id)
--   - tapclicks_instances
--   - sessions, questions, logic_settings, history
--   - estimator_submissions, estimator_questions, estimator_logic, estimator_history
--   - usage_log
--
...
-- Generic updated_at maintenance trigger function (used by multiple tables)
...
-- Security-definer role check — avoids RLS recursion when policies call this
...
CREATE INDEX IF NOT EXISTS idx_instances_user_id ON public.instances USING btree (user_id);

CREATE INDEX IF NOT EXISTS idx_instances_cookie_expires ON public.instances (cookie_expires_at)
  WHERE cookie_expires_at IS NOT NULL;

-- updated_at trigger (safe to replace if already exists)
...
-- Backfill 'user' role for any existing profiles that don't have one yet
INSERT INTO public.user_roles (user_id, role)
...
-- Records are immutable once created (UPDATE is explicitly blocked by policy)
-- -----------------------------------------------------------------------------
...
-- =============================================================================
-- SECTION 6: Auth triggers (fire on auth.users INSERT)
-- =============================================================================
...
-- =============================================================================
-- SECTION 7: Row Level Security — enable on all new tables
-- =============================================================================
...
-- -----------------------------------------------------------------------------
-- instances (patched table — new policies scoped by user_id)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- extractions (immutable — UPDATE is explicitly blocked)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- instance_folders (access controlled via folder ownership)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- migration_stages (access controlled via project ownership)
-- -----------------------------------------------------------------------------
...
-- =============================================================================
-- SECTION 9: Storage bucket for AI file uploads
-- =============================================================================
